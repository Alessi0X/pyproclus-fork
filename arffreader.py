...
def readarff(fileName, supervision=True):
    ...
    f = open(fileName, "r")
    ndim = len(
        re.findall("@attribute", f.read())
    )  # try to guess the number of dimensions
    f.close()

    if supervision:
        X = customload(fileName, usecols=np.arange(ndim - 1))
        try:
            supervision = customload(fileName, usecols=[ndim - 1]).astype(int)
        except:
            supervision = customload(fileName, usecols=[ndim - 1], dtype=str)
            supu = np.unique(supervision)
            supervision = np.array(
                [np.where(supu == i)[0] for i in supervision]
            ).astype(int)
...
def customload(fileName, usecols, dtype=np.double):
    return np.loadtxt(
        fileName, dtype=dtype, comments="@", delimiter=",", usecols=usecols
    )
